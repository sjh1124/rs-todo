...
from models.tfi_nts import TFI_NTS
...
def get_model(name, categorical_field_dims, numerical_num, task_num, expert_num, embed_dim):
    ...
    if name == 'sharedbottom':
        ...
        print("Nodel: TFI_NTS")
        return TFI_NTS(categorical_field_dims, numerical_num, embed_dim=embed_dim, bottom_mlp_dims=(512, 256), tower_mlp_dims=(128, 64), task_num=task_num, expert_num=expert_num, dropout=0.2)
    else:
...
def TFI_NTS_train(model, optimizer, data_loader, criterion, device, log_interval=100):
    model.train()
    total_loss = 0
    loader = tqdm.tqdm(data_loader, smoothing=0, mininterval=1.0)
    alpha = [torch.tensor(0.5, requires_grad=False,dtype=torch.float32), torch.tensor(0.5, requires_grad=False,dtype=torch.float32)]
    # alpha = [0.5, 0.5]
    for i, (categorical_fields, numerical_fields, labels) in enumerate(loader):
        categorical_fields, numerical_fields, labels = categorical_fields.to(device), numerical_fields.to(
            device), labels.to(device)

        y = model(alpha, categorical_fields, numerical_fields)
        loss_list = [criterion(y[i], labels[:, i].float()) for i in range(labels.size(1))]

        grad_ctr = torch.autograd.grad(loss_list[0], model.expert.parameters(), retain_graph=True, allow_unused=True)
        grad_cvr = torch.autograd.grad(loss_list[1], model.expert.parameters(), retain_graph=True, allow_unused=True)
        similarity = sum((g1 * g2).sum() for g1, g2 in zip(grad_ctr, grad_cvr)) / (
                torch.norm(torch.stack([torch.norm(g) for g in grad_ctr])) * torch.norm(
            torch.stack([torch.norm(g) for g in grad_cvr]))
        )

        # 如果梯度相似性为负，说明发生负迁移，调整共享特征和专属特征的比重
        if similarity < 0:
            alpha[0] = min(1.0, alpha[0] + 0.1)  # 增加CTR任务对专属特征的依赖
            alpha[1] = min(1.0, alpha[1] + 0.1)  # 增加CVR任务对专属特征的依赖
        else:
            alpha[0] = max(0.5, alpha[0] - 0.05)  # 若无负迁移，稍微增加共享特征依赖
            alpha[1] = max(0.5, alpha[1] - 0.05)  # 保持alpha在合理范围内
        loss = 0
        for item in loss_list:
            loss += item
        loss /= len(loss_list)
        ...
        total_loss += loss.item()
        # 动态门控控制与恢复机制
        if similarity < -0.5:  # 如果负迁移显著，进行恢复机制
            for param_group in optimizer.param_groups:
                param_group['lr'] = max(param_group['lr'] * 0.9, 1e-5)  # 降低学习率以恢复稳定性
        if (i + 1) % log_interval == 0:
            loader.set_postfix(loss=total_loss / log_interval)
            total_loss = 0
...
def main(dataset_name,
    ...
    for epoch_i in range(epoch):
        ...
        elif model_name == 'idea1':
            TFI_NTS_train(model, optimizer, test_data_loader, criterion, device)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='idea1', choices=['singletask', 'sharedbottom', 'omoe', 'mmoe', 'ple', 'aitm', 'metaheac', 'idea1'])
